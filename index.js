...
const validateCred = (arr) => {
  ...
  arrayForValidation.push(arr[arr.length - 1]);

  for (let i = arr.length - 3; i >= 0; i-= 2) {
    arrayForValidation.push(arr[i])
  }

  for (let i = arr.length - 2; i >= 0; i-= 2) {
    if (arr[i] * 2 > 9) {
      arrayForValidation.push((arr[i] * 2)-9)
    } else {
      arrayForValidation.push((arr[i] * 2))
    }
  }

    const sum = arrayForValidation.reduce((total, amount) => total + amount);
    if (sum % 10 == 0) {
      return true
    } else {
      return false
